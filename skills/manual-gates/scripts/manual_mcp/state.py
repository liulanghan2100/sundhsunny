...
def record(project: str, node_id: int, status: str, artifact: str, notes: str,
           artifact_meta: dict = None, acceptance_checks: list = None,
           command_results: list = None,
           reviewer: str = "", session_id: str = "",
           verification_mode: str = "legacy") -> dict:
    with _project_lock(project):
        state = load(project)
        ...
        state.setdefault("history", []).append({
            ...
            "status": status,
            "verification_mode": verification_mode,
            "artifact_sha256": (artifact_meta or {}).get("sha256"),
            "command_results": command_results or [],
            "session_id": session_id or None,
            "ts": datetime.now(timezone.utc).isoformat(),
        })
